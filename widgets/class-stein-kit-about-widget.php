		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $logo['logo_image'] ) ) {
				...
				if ( ! empty( $logo['logo_width'] ) ) {
					echo sprintf( '#%s .about-logo img { width: %s; }', esc_html( $this->id ), esc_html( $logo['logo_width'] ) );
				}

				if ( ! empty( $logo['logo_height'] ) ) {
					echo sprintf( '#%s .about-logo img { height: %s; }', esc_html( $this->id ), esc_html( $logo['logo_height'] ) );
				}
				echo '</style>';

				echo wp_sprintf(
					'<a class="about-logo tw-block tw-mb-4" href="%1$s"><img src="%2$s" alt="%3$s"></a>',
					esc_url( home_url() ),
					esc_url( wp_get_attachment_image_url( $logo['logo_image'], 'full' ) ),
					esc_attr( get_bloginfo( 'name' ) )
				);
			} else {
				echo wp_sprintf(
					'<h1 class="about-title tw-text-3xl tw-mb-4"><a href="%1$s">%2$s</a></h1>',
					esc_url( home_url() ),
					esc_attr( get_bloginfo( 'name' ) )
				);
			}
