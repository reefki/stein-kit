                ...
                <?php if (! empty($categories)) : ?>
                    <p>
                        ...
                        <select id="<?php echo esc_attr($this->get_field_id('category')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('category')); ?>">
                            <option value="0"<?php selected($instance['category'] == 0) ;?>><?php echo esc_html__('All', 'stein-kit'); ?></option>
                            <?php foreach($categories as $id => $name) : ?>
                                <option value="<?php echo esc_attr($id); ?>"<?php selected($instance['category'] == $id) ;?>><?php echo esc_html($name); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </p>
                <?php endif; ?>
                        ...
                        <li class="tw-flex tw-mb-6">
                            <?php if(has_post_thumbnail()) : ?>
                                <div class="tw-w-20 tw-mr-4">
                                    <a href="<?php the_permalink(); ?>" class="tw-block tw-bg-overlay tw-aspect-ratio-1/1 tw-relative<?php echo esc_attr(! empty($instance['image_style']) ? ' tw-' . $instance['image_style'] : ''); ?>"><?php the_post_thumbnail('thumbnail', array('class' => 'tw-absolute tw-left-0 tw-top-0 tw-h-full tw-w-full tw-object-cover' . (! empty($instance['image_style']) ? ' tw-' . $instance['image_style'] : ''), 'alt' => the_title_attribute('echo=0'))); ?></a>
                                </div>
                            <?php endif; ?>
